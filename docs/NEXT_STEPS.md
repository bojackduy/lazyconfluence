...
- [ ] Add an explicit browser-open service/hook that opens only the selected canonical page URL.
- [ ] Keep browser opening out of rendering components and never use it automatically.
...
- [ ] Mark `open-browser` and `go-back` available only after tests pass.
