...
- Reader UI and document detail are partial: the TUI renders the main reader, navigator, outline, related panel, scroll behavior, safe document-kind symbols, richer markdown/code/table styling, orphaned local pages, and draft previews; it now reads pages from the local SQLite index, edits selected pages in an in-app local Markdown draft editor, and reviews/applies safe storage-backed drafts to Confluence after explicit approval. Browser-open hooks and broader polish remain.
...
- Local editing is started: CLI drafts can be created from a markdown file or `$EDITOR`, staged, unstaged, discarded, previewed, and diffed against the synced body artifact. The TUI opens an in-app Markdown editor with `e`, saves local drafts, stages/unstages, discards, shows diffs, previews drafts in the reader, shows draft/staged state in the header, and can apply safe storage-backed drafts to Confluence after review with version/hash conflict checks. Broader preservation for opaque Confluence content remains.
