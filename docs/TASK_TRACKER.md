...
- Local editing is started: CLI drafts can be created from a markdown file or `$EDITOR`, staged, unstaged, discarded, previewed, and diffed against the synced body artifact. The TUI opens an in-app Markdown editor with `e`; `Ctrl+T` stages the transient editor buffer, `Esc` closes without changing staged docs, and normal-view Overview shows current-space staged diffs for selecting drafts to apply or discard. Broader preservation for opaque Confluence content remains.
